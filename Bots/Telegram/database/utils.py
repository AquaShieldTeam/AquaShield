...
async def get_sensor_settings(hub_id=None, sensor_id=None, sensor=None) -> str:
    if sensor is None:
    ...
    water_lvl = ["Минимальный", "Средний", "Высокий", "Максимальный"]

    sensor_data = f"""Настройки для датчика #<b>{sensor.id}</b>:
    Хаб №<b>{sensor.hub_id}</b>
    Место: <code>{sensor.location}</code>
    Чувствительность:  <code>{water_lvl[sensor.water_threshold]}</code>
